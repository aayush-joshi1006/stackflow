...
const Page = async ({
  ...
  const query = [
    ...
    Query.offset((+searchPar.page - 1) * 25),
    Query.limit(25),
  ];

  if (searchPar.voteStatus)
    query.push(Query.equal("voteStatus", searchPar.voteStatus));
        ...
        <ul className="flex gap-1">
          <li>
            <Link
              ...
              className={`block w-full rounded-full px-3 py-0.5 duration-200 ${
                !searchPar.voteStatus ? "bg-white/20" : "hover:bg-white/20"
              }`}
            >
              All
            </Link>
          ...
          <li>
            <Link
              ...
              className={`block w-full rounded-full px-3 py-0.5 duration-200 ${
                searchPar?.voteStatus === "downvoted"
                  ? "bg-white/20"
                  : "hover:bg-white/20"
              }`}
            >
